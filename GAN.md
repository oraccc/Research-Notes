...
DeepFake可以用很多种技术来实现，对抗生成网络GAN是其中的一种。然而，DeepFake技术大多用于人脸的篡改，例如将一个人脸的部分特征（嘴唇、眼睛等）放到原始的图像上，以达到攻击的目的。在此前提下，是不需要对背景进行修改的，只需对人脸部分进行定位和操作即可，那么通过检测背景的方式便是行不通的，因为真脸和假脸共享一个背景。

如果不局限于DeepFake，那么GAN网络可以生成一张全新的图片（包括背景和人脸），只需要使用大量的数据集进行训练即可。
